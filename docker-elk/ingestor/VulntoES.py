        ...
        for h in self.root.iter('host'):
            ...
            if h.tag == 'host':
                if 'endtime' in h.attrib and h.attrib['endtime']:
                    dict_item['time'] = time.strftime(
                        '%Y/%m/%d %H:%M:%S', time.gmtime(float(h.attrib['endtime'])))
                ...
                if c.tag == 'address':
                    ...
                    for port in c.getchildren():
                        ...
                        if port.tag == 'port':
                                    ...
                                    if p.attrib['id']:
                                        if p.attrib['output']:
                                            if 'scripts' in dict_item_ports:
                                                dict_item_ports['scripts'][p.attrib['id']
                                                                           ] = p.attrib['output']
                                            else:
                            ...
                            to_upload = merge_two_dicts(
                                dict_item, dict_item_ports)
                            if to_upload['state'] == 'open':
                                self.es.index(
                                    index=self.index_name, doc_type="vuln", body=json.dumps(to_upload))
...
def main():
    # input_file, index_name es_ip_address, report_type, create_sql, create_xml, help
    letters = 'i:I:e:p:r:s:h'
    keywords = ['input-file=', 'index_name=', 'es_ip=',
                'es_port=', 'report_type=', 'static=', 'help']
    try:
